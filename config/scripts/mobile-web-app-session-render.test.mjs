 ...
 * parse.
 *
 * Two defects this file found and the commit beside it fixed, both invisible natively and both a
 * console line rather than a crash:
 *
 ...
 *   lines on the console at mount, and a hardware-back guard that was never armed anyway.
 * - `use-mobile-session-diff-comments.ts` ran `void loadDiffComments()` in an effect with no catch.
 *   The loader tolerates a *refused* `worktree.show` and nothing caught a *rejected* one, so a host
 *   that will not answer put an unhandled rejection on every session mount.
 *
 * Both are asserted as the absence of any page or console error below, which is why that assertion
 * is the strict `toEqual([])` and not a filter.
 *
...
describeRender(
  ...
  () => {
    it('mounts the session screen rather than the unmatched route, with nothing on the console', async () => {
      ...
      // Strict, because two of this closure's defects were exactly a console line: the unguarded
      // `BackHandler` and the uncaught `loadDiffComments` rejection.
